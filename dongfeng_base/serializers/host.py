...
class UpHostsSerializer(TaskResultSerializer):
    def __init__(self, up_hosts: list, corp_id: int, env: int, last_scan_time: datetime.datetime):
...
class OpenPortsSerializer(TaskResultSerializer):
    def __init__(self, ip: str, ports: list, corp_id: int, env: int, last_scan_time: datetime.datetime):
        super().__init__(env=env, last_scan_time=last_scan_time)
        self.ip = ip
